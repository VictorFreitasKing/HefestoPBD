...
from ..services import item_NF_service

#views = Blueprint('views', __name__)

class itens_NF_view():
    def getAll(self):
        itens_NFs = item_NF_service.getAll()
    ...
    def post(self, itens_NF):
        if item_NF_service.cadastrar(itens_NF):
            return make_response(item_NF_service.get_ultimo().toJson(), 201)
        else:
            return make_response({}, 200)
    def get(self, id):
        itens_NF = item_NF_service.get(id)
        if itens_NF is not None:
            return make_response(itens_NF.toJson(), 200)
        else:
            return make_response({}, 200)
    def patch(self, id, itens_NF):
        item_NF_service.editar(codigo=id, item_NF=itens_NF)
