...
from ..services import tabela_de_precos_service

#views = Blueprint('views', __name__)

class tabela_de_precos_view():
    def getAll(self):
        tabela_de_precos = tabela_de_precos_service.getAll()
    ...
    def post(self, servicos):
        if tabela_de_precos_service.cadastrar(servicos):
            return make_response(tabela_de_precos_service.get_ultimo().toJson(), 201)
        else:
            return make_response({}, 200)
    def get(self, id):
        tabela_de_preco = tabela_de_precos_service.get(id)
        if tabela_de_preco is not None:
            return make_response(tabela_de_preco.toJson(), 200)
        else:
            return make_response({}, 200)
    def patch(self, id, servicos):
        tabela_de_precos_service.editar(codigo=id, tabela_de_precos=tabela_de_precos)
