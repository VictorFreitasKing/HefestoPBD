...
def getAll():
    ...
    while data_manager is not None:
        lista.append(funcionario.Funcionario(matricula=data_manager[0], nome=data_manager[1], cpf=data_manager[2],  rg=data_manager[3], telefone=data_manager[4], celular=data_manager[5], pais=data_manager[6], estado=data_manager[7],cidade=data_manager[8], bairro=data_manager[9], logradouro=data_manager[10], data_admissao=data_manager[11], data_demissao=data_manager[12], salario=data_manager[13], senha=data_manager[14]))
        data_manager = cursor.fetchone()
